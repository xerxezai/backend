...
class Migration(migrations.Migration):
    ...
    # feedback_migration_rewrites_are_dangerous). The Commission model — which needs a
    # dependency on 'sales' for its order FK — lives in a later migration instead, where
    # declaring a new dependency is safe: a migration name that was never previously applied
    # anywhere can't create the kind of history-order conflict a same-named-but-rewritten 0001
    # did (InconsistentMigrationHistory: mlm.0001 recorded as applied before a 'sales'
    # migration that didn't exist yet at that time — this took the whole site down once
    # already, see incident notes).
    dependencies = [
